...
var baseDir = "../go-apixt/web"

func getApixtJs() (string, error) {
	if apixtJs == "" {
		js, err := os.ReadFile(filepath.Join(baseDir, "apixt.js"))
		if err != nil {
			return "", err
		}
		apixtJs = string(js)
...
func GetResolvedTemplate() (string, error) {
	...
	css, err := os.ReadFile(filepath.Join(baseDir, "index.css"))
	if err != nil {
		return "", err
	}
	js, err := os.ReadFile(filepath.Join(baseDir, "index.js"))
	if err != nil {
		return "", err
	}

	tmpl, err := template.ParseFiles(
		filepath.Join("..", "go-apixt", "templates", "index.html"),
	)
	if err != nil {
		return "", err
	}
